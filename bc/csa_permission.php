...
if ($submit=='update' && $update_id>0) {
	...
	$sql = "UPDATE csa_permission SET
		is_write = '$is_w', 
		is_manage = '$is_m'
	WHERE csa_permission_id = '$update_id' ";
	...
	if($add_user_code != '') {
		...
		if ($row2 = mysqli_fetch_array($result2)) {	
			...
			$sql = "INSERT INTO csa_permission (user_id, is_write, is_manage, create_date) VALUES 
			('$uid', '$add_w', '$add_m', now()) ";
			$q = mysqli_query($connect, $sql);
			$qx = ($qx and $q);	
	...
	$sql = "DELETE FROM csa_permission WHERE csa_permission_id = '$del_id' ";
	$q = mysqli_query($connect, $sql);
	$qx = ($qx and $q);	
	...
	$sql2="SELECT * 
			FROM csa_permission 
			JOIN user ON csa_permission.user_id = user.user_id
			WHERE csa_permission_id = '$edit_id' ";
	$result2=mysqli_query($connect, $sql2);
	if ($row2 = mysqli_fetch_array($result2)) {	
?>
	...
	<div class="col-lg-12 col-xs-12 col-sm-12">
		<div class="portlet light tasks-widget bordered">
			<div class="portlet-title">
				...
				<div class="actions">
					<button type='button' class="btn btn-primary" onClick="document.location='csa_permission.php'"><i class='fa fa-arrow-circle-left'></i> ย้อนกลับ</button>
				</div>
			</div>
			

	
	<form method='post' action='csa_permission.php'>  
...
<div class="row">
	...
	<form method='post' action='csa_permission.php' enctype="multipart/form-data">  
	<div class="form-group">
...
<tr>
	<td><?=$n++?></td>
	<td><a href='csa_permission.php?edit_id=<?=$row['csa_permission_id']?>'><?=$row['code']?></a></td>
	<td><a href='csa_permission.php?edit_id=<?=$row['csa_permission_id']?>'><?=$row['prefix']?><?=$row['name']?> <?=$row['surname']?></a></td>
	<td><?if ($row['is_write']==1) echo '<i class="fa fa-check"></i>'?></td>
	<td><?if ($row['is_manage']==1) echo '<i class="fa fa-check"></i>'?></td>
	<td align='right'><a href="csa_permission.php?del_id=<?=$row['csa_permission_id']?>" onclick='return confirm("Confirm Delete?");'>ลบ</a></td>
</tr>
<?
				}
			} else {
