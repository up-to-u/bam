...
<?
		$sql="SELECT 
		department.*,
		(SELECT COUNT(*) FROM job_function WHERE job_function.department_id2 = department.department_id AND mark_del = '0') AS num		
		FROM department 
		WHERE 
...
<?
		$sql="SELECT 
			department.*,
			(SELECT COUNT(*) FROM job_function WHERE job_function.department_id3 = department.department_id AND mark_del = '0') AS num
		FROM department 
		WHERE 
		department.parent_id = '$parent' AND 
		department.mark_del = '0' 
...
<?
		$sql="SELECT 
		department.*,
		(SELECT COUNT(*) FROM csa_audit WHERE csa_year = '$y' AND csa_audit.department_id2 = department.department_id AND mark_del = '0') AS num		
		FROM department 
		WHERE 
		department.parent_id = '$parent' AND 
		department.mark_del = '0' 
...
<?
		$sql="SELECT 
			department.*,
			(SELECT COUNT(*) FROM csa_audit WHERE csa_year = '$y' AND csa_audit.department_id3 = department.department_id AND mark_del = '0') AS num
		FROM department 
		WHERE 
		department.parent_id = '$parent' AND 
		department.mark_del = '0' 
...
<?
		$sql="SELECT 
			department.*,
			(SELECT COUNT(*) FROM user WHERE mark_del = 0 AND department_id = department.department_id AND mark_del = '0') AS num
		FROM department 
		WHERE 
		department.parent_id = '$parent' AND 
		department.mark_del = '0' 
...
<?	}
	...
	if ($lv==1 && $parent>0) {
?>
	<select id='csa_factor1' class="form-control" required <?if ($is_disable=='disabled') echo 'disabled'?>>
...
<?
	...
	while ($row1 = mysqli_fetch_array($result1)) {
?>		
		<option value='<?=$row1['csa_factor_id']?>' <?if ($row1['csa_factor_id']==$data) echo 'selected'?>><?=$row1['factor_no'].' '.$row1['factor']?></option>
<?	} ?>
	</select>
<?	
	} else if ($lv==2 && $parent>0) {
?>
	<select id='csa_factor2' name='csa_factor' class="form-control" required <?if ($is_disable=='disabled') echo 'disabled'?>>
		<option value=''>--- เลือก ---</option>
<?		
	$sql="SELECT * FROM csa_factor WHERE parent_id='$parent' AND mark_del='0'  ORDER BY factor_no, csa_factor_id";
...
<?	} else {
		echo '<select class="form-control" required disabled><option>--- โปรดเลือกตัวเลือกก่อน ---</option></select>';
	}
	exit;
...
?>
			  ...
			  <select name='csa_factor' id='csa_factor' class="form-control" required <?if ($is_disable=='disabled') echo 'disabled'?>>
				<option value=''>--- เลือก ---</option>
<?
	$sql="SELECT * FROM csa_factor WHERE csa_risk_type_id='$parent' AND mark_del='0' ORDER BY factor_no, csa_factor_id";
	$result1=mysqli_query($connect, $sql);
